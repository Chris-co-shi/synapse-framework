...
3. MyBatis-Plus 可以完整使用，但 Entity、ServiceImpl、ActiveRecord 不允许直接暴露到 Controller 或前端。
4. Entity 只允许作为持久化模型使用。
5. Repository Port / Service 边界必须面向 application/domain 暴露。
6. Repository Adapter 或 ServiceImpl 必须封装 MyBatis-Plus 细节。
7. 基础 CRUD 可以使用 IService / ServiceImpl，跨模块编排仍放 application service。
8. Entity 可以按 MyBatis-Plus 官方实践继承 Model<T>，但不允许成为接口响应对象。
...
- MyBatis-Plus 模型是否直接暴露到 Controller 或前端：
- 是否使用 IService / ServiceImpl：
- 如使用，是否只用于持久化或基础 CRUD：
