...
`synapse-messaging` 定义跨 Broker 的消息外壳、发布/消费 Port、上下文传播和消费结果分类，使 RocketMQ、Kafka、RabbitMQ 等 adapter 可以共享统一语义，而 Framework 核心不依赖任何真实 Broker SDK。
...
MessageEnvelope / HeaderKeys
  ...
  -> MessageConsumeTemplate
  -> SynapseMessagingAutoConfiguration
